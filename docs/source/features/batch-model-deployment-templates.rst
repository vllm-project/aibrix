...
defaults and then built-in defaults. ``adaptive_healthy_window`` is the minimum
number of healthy completions in a growth window (default 8).
...
overload or latency-driven decrease, it uses ``adaptive_additive_increase`` as
the fixed AIMD congestion-avoidance step (default 1). Lower minimum windows
speed up low-concurrency probing, while larger increases ramp up faster but can
overshoot backend capacity. Operators can set their process-wide fallbacks with
