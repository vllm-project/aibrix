...
``authoritativeRoutingPolicy``. The gateway then ignores the request's
``routing-strategy``, ``config-profile`` (including ``auto``), and
``external-filter`` headers. It uses ``defaultProfile``, preserves the routing
headers that the gateway itself generates after selecting a pod, and omits
...
.. list-table::
   ...
   * - ``authoritativeRoutingPolicy``
     - When ``true``, ignores application-supplied ``routing-strategy``, ``config-profile`` and ``external-filter`` headers. The gateway resolves ``defaultProfile``, retains its internally generated routing headers, and omits routing diagnostic headers from the client response. Defaults to ``false`` for backward compatibility.
   * - ``defaultProfile``
...
2. ``routing-strategy`` request header, unless ``authoritativeRoutingPolicy`` is ``true``.
