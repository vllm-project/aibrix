	...
	err := wait.PollUntilContextTimeout(ctx, rayClusterPollInterval, rayClusterPollTimeout, true,
		func(ctx context.Context) (bool, error) {
			for i := range clusters {
				...
				ready, err := h.headPodReady(ctx, current.UID)
				if err != nil {
					return false, retryableRayClusterAPIError(err)
				}
				if !rayClusterReady(current) || !ready {
					return false, nil
				}
			}
			return true, nil
...
func (h *rayClusterHarness) headPodReady(ctx context.Context, clusterUID types.UID) (bool, error) {
