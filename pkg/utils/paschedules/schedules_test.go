...
func TestResolveScheduledBounds(t *testing.T) {
	...
	assert.Equal(t, Bounds{MinReplicas: 3, MaxReplicas: 12, ActiveSchedule: "business-hours"}, bounds)
...
func TestResolveUsesUTCAndEveryDayDefaults(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, Bounds{MinReplicas: 2, MaxReplicas: 10, ActiveSchedule: "utc-window"}, bounds)
}
