...
func TestParseModelConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name: "single profile",
			json: `{"profiles":{"default":{"routingStrategy":"pd","promptMinLength":0,"promptMaxLength":2048}}}`,
		},
		{
			name: "multiple profiles with defaultProfile",
			json: `{"defaultProfile":"pd","profiles":{"default":{"routingStrategy":"random","promptMinLength":0,"promptMaxLength":4096},"pd":{"routingStrategy":"pd","promptMinLength":0,"promptMaxLength":2048}}}`,
		},
		{
			name: "with combined field",
			json: `{"profiles":{"default":{"routingStrategy":"pd","promptMinLength":0,"promptMaxLength":2048,"combined":true}}}`,
		},
		{
			name:    "invalid json",
			json:    `{`,
...
func TestParseModelConfig_DefaultValues(t *testing.T) {
	// promptMinLength negative → normalized to 0
	// promptMaxLength 0 or omitted → MaxInt32
	json := `{"profiles":{"p1":{"routingStrategy":"pd","promptMinLength":-5,"promptMaxLength":0},"p2":{"routingStrategy":"random"}}}`
	...
	if p1.PromptMinLength != 0 {
		t.Errorf("promptMinLength -5 should be normalized to 0, got %d", p1.PromptMinLength)
	}
	if p1.PromptMaxLength != math.MaxInt32 {
		t.Errorf("promptMaxLength 0 should become MaxInt32, got %d", p1.PromptMaxLength)
	}
	...
	if p2.PromptMaxLength != math.MaxInt32 {
		t.Errorf("omitted promptMaxLength should become MaxInt32, got %d", p2.PromptMaxLength)
	}
}
...
func TestResolveProfileFromPod(t *testing.T) {
	configJSON := `{"defaultProfile":"pd","profiles":{"default":{"routingStrategy":"random"},"pd":{"routingStrategy":"pd","promptMaxLength":2048}}}`
...
func TestResolveProfile(t *testing.T) {
	configJSON := `{"defaultProfile":"pd","profiles":{"default":{"routingStrategy":"random","promptMinLength":0,"promptMaxLength":4096},"pd":{"routingStrategy":"pd","promptMinLength":0,"promptMaxLength":2048}}}`
