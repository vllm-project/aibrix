...
func (r *pdRouter) chargeTokenLoad(routingCtx *types.RoutingContext, pod *v1.Pod, policy pd.PrefillScorePolicy, scorer pd.PrefillScorer) {
	...
	r.tokenLoadTracker.AcquirePrefillWithTTL(routingCtx.RequestID, pd.PodKey(pod), cost, overrides.TokenLoad.TTL)
}
...
func (r *pdRouter) filterPrefillDecodePods(routingCtx *types.RoutingContext, readyPods []*v1.Pod) (*v1.Pod, *v1.Pod, error) {
	...
	if bucketing {
		if len(promptLengthBucketingPrefillPods) == 0 || len(promptLengthBucketingDecodePods) == 0 {
			...
			if combinedAvailable {
				...
				r.pendingDecodeTracker.AddPendingDecode(routingCtx.RequestID, pd.PodKey(combinedPod))
	...
	r.pendingDecodeTracker.AddPendingDecode(routingCtx.RequestID, pd.PodKey(selectedDecode))
	r.prefillRequestTracker.AddPrefillRequest(routingCtx.RequestID, pd.PodKey(selectedPrefill))
...
func (r *pdRouter) scoreDecodePods(routingCtx *types.RoutingContext, filteredDecodePods []*v1.Pod,
	...
	for _, pod := range filteredDecodePods {
		...
		if anyMetricsReady && !metricsReadyByPod[pod.Name] {
			...
			pending := float64(r.pendingDecodeTracker.GetPendingDecodeCount(pd.PodKey(pod)))
