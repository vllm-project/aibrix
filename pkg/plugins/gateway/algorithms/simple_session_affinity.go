...
func (r *sessionAffinityRouter) Route(ctx *types.RoutingContext, readyPodList types.PodList) (string, error) {
	if ctx.ReqHeaders == nil {
		klog.V(4).Info("No request or headers, skipping session affinity")
	...
	if targetAddr != "" {
		for _, pod := range readyPodList.All() {
			...
			if addr == targetAddr {
				...
				klog.V(4).Infof("Session affinity matched address: %s", addr)
	...
	klog.V(4).Infof("Session affinity failed (session=%q), falling back", sessionID)
	return r.fallbackRoute(ctx, readyPodList)
}
