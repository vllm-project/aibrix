...
//
// It never blocks: the abort POST runs in its own goroutine on a context
// derived from context.Background() (the request context is typically already
// cancelled or about to be) with its own AIBRIX_DECODE_ABORT_TIMEOUT budget.
// Nothing downstream waits for, or branches on, the abort result.
//
...
func OnPrefillLegFailed(client *http.Client, leg *types.PDLegState, requestID, model string, err error) *types.PrefillFailure {
	...
	go func() {
		...
		attemptAbort := func(attempt int) {
			...
			if abortErr := postDecodeAbort(client, decodeAddr, rid, timeout); abortErr != nil {
...
//
// The context is deliberately rooted at context.Background(): the request
// context that carried the prefill leg is usually already cancelled by the
// time a prefill failure is observed, and the abort must still go out.
func postDecodeAbort(client *http.Client, decodeAddr, rid string, timeout time.Duration) error {
	if client == nil {
