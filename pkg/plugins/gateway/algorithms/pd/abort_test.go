...
func newAbortRecorder() *abortRecorder {
	return &abortRecorder{got: make(chan string, 8)}
}
...
func TestPostDecodeAbortIgnoresRequestCancellation(t *testing.T) {
	...
	require.NoError(t, postDecodeAbort(decodeSrv.Client(), addr, "rid-1", 2*time.Second))
	...
	defer bad.Close()
	err := postDecodeAbort(bad.Client(), strings.TrimPrefix(bad.URL, "http://"), "rid-2", time.Second)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), strconv.Itoa(http.StatusServiceUnavailable))

	assert.Error(t, postDecodeAbort(nil, addr, "rid-3", time.Second), "a missing client is an error, not a panic")
}
...
// landed after all and the second abort must not go out.
func TestAbortRetrySkippedWhenDecodeStartsStreaming(t *testing.T) {
	withAbortRetryDelay(t, 300*time.Millisecond)
	...
	leg.MarkDecodeResponded()
	recorder.none(t, 900*time.Millisecond)
}
