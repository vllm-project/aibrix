...
func TestCalculateAchievableCanaryReplicas(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedAchievable:     2, // currentUpdated(1) + maxUnavailable(1) = 2
		},
		{
			...
			expectedAchievable:     4, // currentUpdated(2) + maxUnavailable(2) = 4
		},
		{
			...
			expectedAchievable:     3, // min(3, 3+1) = 3
		},
		{
			...
			currentUpdatedRoleSets: 2,
			expectedAchievable:     5, // min(5, min(5, 2+10)) = 5
		},
	}
...
func TestActualVsAchievableReplicaCounting(t *testing.T) {
	tests := []struct {
		...
		expectedCanaryReplicas int32 // What status should show (actual)
		expectedStableReplicas int32 // What status should show
	...
	}{
		{
			...
			currentUpdatedRoleSets: 1,
			maxUnavailable:         1,
			expectedCanaryReplicas: 1, // Actual current state
			expectedStableReplicas: 9, // 10 - 1
			expectedAchievable:     2, // 1 + 1 = 2 (constraint applied)
		},
		{
			...
			currentUpdatedRoleSets: 3,
			maxUnavailable:         1,
			expectedCanaryReplicas: 3, // Actual current state
			expectedStableReplicas: 7, // 10 - 3
			expectedAchievable:     4, // 3 + 1 = 4 (constraint applied)
		},
		{
			...
			maxUnavailable:         1,
			expectedCanaryReplicas: 8, // Actual current state
			expectedStableReplicas: 2, // 10 - 8
			expectedAchievable:     9, // 8 + 1 = 9 (still constrained)
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify status shows actual current state
			assert.Equal(t, tt.expectedCanaryReplicas, stormService.Status.CanaryStatus.CanaryReplicas, "CanaryReplicas should show actual current state")
			assert.Equal(t, tt.expectedStableReplicas, stormService.Status.CanaryStatus.StableReplicas, "StableReplicas should show actual stable count")
