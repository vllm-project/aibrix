...
func TestProcessCanaryPauseStep_AutomaticPause(t *testing.T) {
	...
	// Duration field is accepted but ignored - all pauses are now manual
	...
	require.NoError(t, err)
	assert.False(t, result.Requeue)
	// All pauses are manual now, so no automatic requeue
...
func TestProcessCanaryPauseStep_ManualPause(t *testing.T) {
	...
	// For manual pause, we should expect an event
