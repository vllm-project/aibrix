locals {
  ...
  model_yaml = { for index, resource in provider::kubernetes::manifest_decode_multi(data.http.model.response_body) : "${resource.kind}/${resource.metadata.name}" => yamlencode(resource) }
}