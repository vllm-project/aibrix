import { Check, Copy, Download, Loader2, Pencil, RefreshCw, Sparkles, Square, User, Volume2, X } from 'lucide-react'
...
import { type CreationMessage, cacheMessages, getCachedMessages, pendingCreation } from '@/app/creation-store'
...
export function ChatPage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: messages triggers scroll on new content
  useEffect(() => {
    scrollToBottom()
        ...
        if (conv) {
          ...
          const apiMsgs: Message[] = (conv.messages ?? [])
            ...
            .map((m: ApiMessage) => ({
              ...
              attachments: ((m as ApiMessage & { attachments?: MessageAttachment[] }).attachments ?? []).map(
  ...
  const pollVideoStatus = useCallback(async (jobId: string, assistantMsgId: string) => {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantMsgId && m.role === 'assistant' ? { ...m, videoJob: status, loading: false } : m,
          ),
        )
  ...
  const handleImageGeneration = useCallback(
    async (prompt: string, model: string, ratio: string, referenceFile?: File, referenceImageUrl?: string) => {
      ...
      try {
        ...
        if (referenceFile) {
          ...
          const editSize =
            isDallE2 && !['256x256', '512x512', '1024x1024'].includes(sizeMap.image) ? '1024x1024' : sizeMap.image
          response = await editImage(referenceFile, prompt, model, editSize, 1)
        } else {
          response = await generateImage(prompt, model, sizeMap.image, 1)
        }
        setMessages((prev) =>
          prev.map((m) => (m.id === assistantMsgId ? { ...m, images: response.data, loading: false } : m)),
        )
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : 'Image generation failed'
        setMessages((prev) => prev.map((m) => (m.id === assistantMsgId ? { ...m, error: msg, loading: false } : m)))
      }
    },
    [],
  )
          ...
          prev.map((m) =>
            m.id === assistantMsgId
              ? {
                  ...m,
                  videoJob: job,
                  loading: job.status !== 'completed' && job.status !== 'failed',
                }
              : m,
          ),
        )
  ...
  const sendMessage = useCallback(
    (content: string, model: string, attachments?: InputAttachment[]) => {
      ...
      const attachmentPayload: ChatAttachmentPayload[] | undefined = attachments?.map((attachment) => ({
        id: attachment.id,
  ...
  useEffect(() => {
    if (!id || loading || processedFirstMsg.current || !location.state) return
    ...
    if (mode === 'image') {
      handleImageGeneration(firstMessage, model, ratio ?? '1:1', referenceFile, referenceImageUrl)
    } else if (mode === 'video') {
      handleVideoGeneration(firstMessage, model, ratio ?? '1:1')
    } else {
      sendMessage(firstMessage, model, attachments)
    }
  }, [id, loading, location.state, sendMessage, handleVideoGeneration, handleImageGeneration])
  ...
  const handleEditSave = async () => {
    ...
    const model = messages.find((m) => m.model)?.model || lastModelRef.current
  ...
  const handleRegenerate = (msgId: string) => {
    ...
    const model = messages[idx].model || messages.find((m) => m.model)?.model || lastModelRef.current
  ...
  const renderCreationUserMessage = (msg: Message) => (
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center gap-2 mt-1.5">
        ...
        <span className="text-xs text-muted-foreground">&middot;</span>
        <span className="text-xs text-muted-foreground capitalize">{msg.mode}</span>
      </div>
    </div>
  )
    ...
    return (
      <div className="mt-3 flex flex-wrap gap-2">
        {attachments.map((attachment) => (
          <div key={attachment.id} className="w-[120px] rounded-xl overflow-hidden border border-border bg-accent/50">
            {attachment.kind === 'image' && attachment.blob_url ? (
              <img src={attachment.blob_url} alt={attachment.name} className="w-full h-[90px] object-cover" />
            ) : (
      ...
      {msg.loading && (
        <div className="w-full max-w-[320px] aspect-square rounded-lg bg-accent/50 border border-border flex items-center justify-center">
          <div className="text-center px-6">
            ...
            <p className="text-xs text-muted-foreground">Creating your {msg.mode}...</p>
          </div>
        </div>
      )}
        ...
        <div className="flex gap-3 flex-wrap">
          {msg.images.map((img, idx) => (
            <div key={idx} className="relative group">
              ...
              {img.revised_prompt && (
                <p className="text-xs text-muted-foreground mt-1.5 max-w-[320px]">{img.revised_prompt}</p>
              )}
            </div>
          ))}
        </div>
          ...
          {msg.videoJob.status === 'completed' ? (
            ...
            <>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-foreground capitalize">{msg.videoJob.status}</span>
                {msg.videoJob.progress != null && msg.videoJob.progress > 0 && (
                  <span className="text-xs text-muted-foreground">{Math.round(msg.videoJob.progress)}%</span>
                )}
              </div>
              {msg.videoJob.error && <p className="text-sm text-red-400">{msg.videoJob.error}</p>}
            </>
          )}
        </div>
      )}
              ...
              <div key={msg.id} className="group flex gap-3">
                <div
                  className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center mt-0.5 ${
                    msg.role === 'user' ? 'bg-accent' : isCreation ? 'bg-amber-400/15' : 'bg-orange-800/30'
                  }`}
                >
                  {msg.role === 'user' ? (
                ...
                {isCreation ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-muted-foreground mb-1">{msg.role === 'user' ? 'You' : 'AI Assistant'}</p>
                    {msg.role === 'user' ? (
                      <>
                        {editingId === msg.id ? (
