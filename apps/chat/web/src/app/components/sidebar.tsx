    ...
    <>
      <div
        className={`flex flex-col h-full bg-sidebar text-sidebar-foreground transition-all duration-300 ${
          collapsed ? 'w-0 overflow-hidden' : 'w-[220px] min-w-[220px] overflow-visible'
        }`}
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto mt-4 px-3 space-y-4">
          {starredChats.length > 0 && (
            <div>
              <p className="px-2 mb-1.5 text-xs text-sidebar-foreground/40 tracking-wide uppercase">Starred</p>
              <div className="flex flex-col gap-0.5">{renderChatList(starredChats)}</div>
            </div>
          )}

          {recentChats.length > 0 && (
            <div>
              <p className="px-2 mb-1.5 text-xs text-sidebar-foreground/40 tracking-wide uppercase">Recents</p>
              <div className="flex flex-col gap-0.5">{renderChatList(recentChats)}</div>
            </div>
          )}
        </div>
