import { ArrowUp, Check, ChevronDown, Image as ImageIcon, Paperclip, RatioIcon, Sparkles, Video, X } from 'lucide-react'
import { useCallback, useEffect, useRef, useState } from 'react'
import { useNavigate } from 'react-router'
import { createConversation, fetchModels, type ModelInfo, notifyConversationsChanged } from '../../api/client'
...
const FALLBACK_VIDEO_MODELS: ModelInfo[] = [{ id: 'sora-2', name: 'sora-2', capabilities: ['video'], owned_by: null }]
...
export function AICreationPage() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchModels('image'), fetchModels('video')]).then(([img, vid]) => {
      if (cancelled) return
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>, isReference = false) => {
  ...
  const canGenerate = (prompt.trim().length > 0 || uploadedFiles.length > 0) && !generating
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 pb-20">
        ...
        <p className="text-sm text-muted-foreground mb-8">Let creation come with inspiration</p>
          ...
          <div className="bg-card border border-border rounded-2xl">
            ...
            {referenceImages.length > 0 && (
              <div className="px-4 pt-3 pb-1">
                <p className="text-xs text-muted-foreground mb-1.5">Reference images</p>
                <div className="flex gap-2 flex-wrap">
            ...
            <div className="flex items-center justify-between px-3 pb-3">
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  <span>Ref image</span>
                  {referenceImages.length > 0 && (
                    <span className="text-xs bg-amber-400/20 px-1.5 rounded-full">{referenceImages.length}</span>
                  )}
                </button>
                        ...
                        {models.map((model) => (
                          ...
                          >
                            ...
                            {selectedModel === model.id && (
                              <Check size={14} className="text-foreground/70 flex-shrink-0" />
                            )}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="p-1.5">
                        {aspectRatios.map((ratio) => (
                          ...
                          >
                            ...
                            {selectedRatio === ratio.value && <Check size={14} className="text-foreground/70" />}
                          </button>
                        ))}
                      </div>
                    </div>
