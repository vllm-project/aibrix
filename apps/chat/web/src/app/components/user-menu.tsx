...
type LearnMoreItem = { label: string; href: string; shortcut?: string } | { type: 'separator' }
...
export function UserMenu({ userName = 'User', userEmail = '', planName = '' }: UserMenuProps) {
  const { logout: authLogout } = useAuth()
  const [isOpen, setIsOpen] = useState(false)
  const [activeSubmenu, setActiveSubmenu] = useState<'language' | 'learnMore' | null>(null)
    ...
    <div ref={containerRef} className="relative px-3 pb-3">
          ...
          <div className="rounded-xl border border-white/10 shadow-2xl" style={{ backgroundColor: '#352f29' }}>
                        ...
                        >
                          <span className="w-5 flex-shrink-0">
                            {selectedLanguage === lang.code && (
                              <Check size={16} className="text-sidebar-foreground/60" />
                            )}
                          </span>
            ...
            <div className="border-t border-white/5 py-1">
              ...
              <MenuItem
                ...
                onMouseEnter={() => setActiveSubmenu(null)}
              />
              <div className="relative" onMouseEnter={() => setActiveSubmenu('learnMore')}>
                <MenuItem
                      ...
                      {learnMoreLinks.map((item, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.shortcut ? (
                              <span className="text-xs text-sidebar-foreground/40 ml-4">{item.shortcut}</span>
                            ) : item.href !== '#' ? (
                              <ExternalLink size={14} className="text-sidebar-foreground/40 ml-4" />
                            ) : null}
                          </a>
                        )
                      })}
      ...
      >
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          <div className="text-xs text-sidebar-foreground/40 truncate leading-tight">{planName}</div>
        </div>
        <ChevronUp
          size={14}
...
function MenuItem({ icon, label, shortcut, hasSubmenu, isActive, onClick, onMouseEnter }: MenuItemProps) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      {shortcut && <span className="text-xs text-sidebar-foreground/30">{shortcut}</span>}
      {hasSubmenu && <ChevronRight size={14} className="text-sidebar-foreground/40" />}
    </button>
  )
}
