...
export function ChatItem({
  ...
  return (
    <div className="relative group">
      ...
      >
        {chat.starred && <Star size={12} className="text-amber-400 mr-1.5 flex-shrink-0" fill="currentColor" />}
      ...
      <div ref={menuRef}>
        <button
          ...
          className={`absolute right-1 top-1/2 -translate-y-1/2 p-0.5 rounded-md transition-all ${
            menuOpen ? 'opacity-100 bg-sidebar-accent' : 'opacity-0 group-hover:opacity-100 hover:bg-sidebar-accent'
          }`}
        >
              ...
              <div key={item.label}>
                ...
                <button
                  ...
                  className={`flex items-center gap-2.5 w-full px-3 py-1.5 text-sm transition-colors ${
                    item.isDanger ? 'text-red-400 hover:bg-red-500/10' : 'text-foreground hover:bg-accent'
                  }`}
                >
                  <item.icon
                    size={15}
                    fill={'iconFill' in item ? (item.iconFill as string) : 'none'}
