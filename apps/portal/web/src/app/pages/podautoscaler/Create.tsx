...
export default function PodAutoscalerCreate() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-2xl font-bold text-[var(--foreground)] mb-6">Create PodAutoscaler</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-[var(--card)] p-6 rounded-lg border border-[var(--border)]">
        ...
        <div className="border border-[var(--border)] rounded-md p-4 space-y-3">
          <p className="text-sm font-medium text-[var(--foreground)]">Scale Target Ref</p>
          <div>
            <label className="block text-xs font-medium text-[var(--foreground)] mb-1">API Version *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--foreground)] mb-1">Kind *</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Min Replicas</label>
            <input type="number" min={0} value={form.minReplicas ?? 1} onChange={(e) => setForm({ ...form, minReplicas: parseInt(e.target.value) || 1 })}
              className="w-full px-3 py-2 border border-[var(--input-border)] rounded-md text-sm bg-[var(--background)] text-[var(--foreground)] focus:ring-[var(--input-focus)] focus:border-[var(--input-focus)]" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Max Replicas *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Scaling Strategy *</label>
