...
import { kvCacheApi, type KVCacheDetail as DetailType } from "@/api/kvcache";

export default function KVCacheDetail() {
  ...
  useEffect(() => {
    if (namespace && name) {
      kvCacheApi.get(namespace, name).then(setData).catch((e) => setError(e.message));
    }
  }, [namespace, name]);

  const handleDelete = async () => {
    if (!namespace || !name) return;
    if (!confirm(`Delete KVCache "${name}"?`)) return;
    await kvCacheApi.delete(namespace, name);
    navigate("/kvcaches");
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Mode</dt><dd>{data.mode || "-"}</dd></div>
          </dl>
        </div>
