...
import { rayClusterFleetApi, type RayClusterFleetDetail as DetailType } from "@/api/rayclusterfleet";

export default function RayClusterFleetDetail() {
  ...
  useEffect(() => {
    if (namespace && name) {
      rayClusterFleetApi.get(namespace, name).then(setData).catch((e) => setError(e.message));
    }
  }, [namespace, name]);

  const handleDelete = async () => {
    if (!namespace || !name) return;
    if (!confirm(`Delete RayClusterFleet "${name}"?`)) return;
    await rayClusterFleetApi.delete(namespace, name);
    navigate("/rayclusterfleets");
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Replicas</dt><dd>{data.replicas}</dd></div>
          </dl>
        </div>
        ...
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Updated Replicas</dt><dd>{data.status.updatedReplicas}</dd></div>
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Available Replicas</dt><dd>{data.status.availableReplicas}</dd></div>
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Unavailable Replicas</dt><dd>{data.status.unavailableReplicas}</dd></div>
          </dl>
        </div>
      </div>
