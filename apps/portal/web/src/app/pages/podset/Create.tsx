...
import { podSetApi, type PodSetCreateRequest } from "@/api/podset";

export default function PodSetCreate() {
  const navigate = useNavigate();
  const [form, setForm] = useState<PodSetCreateRequest>({
    name: "", namespace: "default", podGroupSize: 1, stateful: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const resp = await podSetApi.create(form);
      navigate(`/podsets/${resp.namespace}/${resp.name}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-2xl font-bold text-[var(--foreground)] mb-6">Create PodSet</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-[var(--card)] p-6 rounded-lg border border-[var(--border)]">
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--foreground)] mb-1">Pod Group Size *</label>
          <input type="number" min={1} required value={form.podGroupSize} onChange={(e) => setForm({ ...form, podGroupSize: parseInt(e.target.value) || 1 })}
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button type="button" onClick={() => navigate("/podsets")}
