...
import { podSetApi, type PodSetDetail as DetailType } from "@/api/podset";
import StatusBadge from "@/app/components/StatusBadge";

export default function PodSetDetail() {
  ...
  useEffect(() => {
    if (namespace && name) {
      podSetApi.get(namespace, name).then(setData).catch((e) => setError(e.message));
    }
  }, [namespace, name]);

  const handleDelete = async () => {
    if (!namespace || !name) return;
    if (!confirm(`Delete PodSet "${name}"?`)) return;
    await podSetApi.delete(namespace, name);
    navigate("/podsets");
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Pod Group Size</dt><dd>{data.podGroupSize}</dd></div>
        ...
        <div className="bg-[var(--card)] rounded-lg border border-[var(--border)] p-5">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between"><dt className="text-[var(--muted-foreground)]">Ready Pods</dt><dd>{data.status.readyPods}/{data.status.totalPods}</dd></div>
          </dl>
        </div>
      </div>
