...
class VLLMInferenceEngine(InferenceEngine):
    def __init__(self, name: str, version: str, endpoint: str, api_key: Optional[str] = None):
    ...
    async def load_lora_adapter(self, request: LoadLoraAdapterRequest) -> Union[ErrorResponse, str]:
        ...
        try:
            ...
            logger.error(f"Failed to load LoRA adapter '{lora_name}' from `{lora_path}` "
                         f"for httpx request failed, {e}")
            return self._create_error_response(f"Failed to load LoRA adapter '{lora_name}'",
    ...
    async def unload_lora_adapter(self, request: UnloadLoraAdapterRequest) -> Union[ErrorResponse, str]:
        ...
        try:
            ...
            logger.error(f"Failed to remove LoRA adapter '{lora_name}' by id `{lora_int_id}`"
                         f"for httpx request failed, {e}")
            return self._create_error_response(f"Failed to unload LoRA adapter '{lora_name}'",
                                               err_type="ServerError", 
                                               status_code=HTTPStatus.INTERNAL_SERVER_ERROR)
        if response.status_code != 200:
            logger.error(f"Failed to remove LoRA adapter '{lora_name}' by id `{lora_int_id}`"
