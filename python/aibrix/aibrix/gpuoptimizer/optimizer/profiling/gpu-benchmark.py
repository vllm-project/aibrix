...
async def benchmark(
    ...
    async for request in get_request(input_requests, request_rate, num_requests):
        ...
        task = asyncio.create_task(send_request(backend, api_url, model, prompt,
                                                prompt_len, output_len, next_in,
...
def main(args: argparse.Namespace):
    ...
    asyncio.run(benchmark(args.backend, api_url, args.model, input_requests, args.best_of,
                          args.use_beam_search, args.request_rate, args.num_prompts))
    ...
    if args.verbose:
        ...
        result["mean"] = sum([output for _, output, _ in REQUEST_LATENCY]) / benchmark_time
        print(json.dumps(result))
    ...
    if args.verbose:
        ...
        result["P50"] = np.percentile([latency for _, _, latency in REQUEST_LATENCY], 50)
        result["P90"] = np.percentile([latency for _, _, latency in REQUEST_LATENCY], 90)
        result["P99"] = np.percentile([latency for _, _, latency in REQUEST_LATENCY], 99)
        print(json.dumps(result))
