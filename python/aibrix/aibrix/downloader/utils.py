...
def meta_file(local_path: Union[Path, str], file_name: str) -> Path:
    return Path(local_path).joinpath(DOWNLOAD_CACHE_DIR).joinpath(f"{file_name}.metadata").absolute()
...
def check_file_exist(local_file: Union[Path, str], 
                     meta_file: Union[Path, str], 
                     expected_file_size: int, 
                     expected_etag: str) -> bool:
