...
class LLMAdaptiveConcurrencySettings:
    ...
    ``healthy_window`` is the minimum number of healthy completions in a growth
    window. ``additive_increase`` is the minimum probe step and the fixed
    congestion-avoidance step after the first decrease.
    """
...
class LLMAdaptiveConcurrencyController:
    ...
    def __init__(
        ...
        *,
        ...
        self._probing = True
        self._healthy = 0
        self._healthy_window_size = self._growth_window_size()
    ...
    def on_complete(self, outcome: ConcurrencyOutcome) -> None:
        ...
        self._healthy += 1
        if self._healthy >= self._healthy_window_size:
            self._healthy = 0
            self._increase()
    ...
    def _growth_window_size(self) -> int:
        return max(self._settings.healthy_window, self._limit)
