...
class TOSArtifactDownloader(ArtifactDownloader):
    ...
    async def download(
        ...
        ak = (
            credentials.get("ak")
            or credentials.get("access-key")
            or credentials.get("access_key")
            or credentials.get("TOS_ACCESS_KEY")
        )
        sk = (
            credentials.get("sk")
            or credentials.get("secret-key")
            or credentials.get("secret_key")
            or credentials.get("TOS_SECRET_KEY")
        )
        session_key = (
            credentials.get("session_key")
            or credentials.get("tos_session_key")
            or credentials.get("TOS_SESSION_KEY", None)
        )
        if not ak or not sk:
            raise ValueError("TOS ak/sk must be provided in credentials dict.")
        ...
        enable_crc_value = credentials.get("enable_crc") or credentials.get("tos_enable_crc")
        enable_crc = (
            str(enable_crc_value).strip().lower() in {"1", "true", "t", "yes", "y", "on"}
            if enable_crc_value is not None
            else False
        )

        client = tos.TosClientV2(
            ak=ak, sk=sk, security_token=session_key, endpoint=endpoint, region=region, enable_crc=enable_crc
        )
        ...
        try:
            if object_key.endswith("/"):
                self._download_tos_directory(client, bucket_name, object_key, local_path)
            else:
                destination_file = os.path.join(local_path, os.path.basename(object_key))
