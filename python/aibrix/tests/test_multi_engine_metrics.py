...
class TestMultiEngineMetricsSupport:
    ...
    def test_get_metric_rules_vllm(self):
        ...
        assert rules["vllm:gpu_cache_usage_perc"].new_name == "aibrix:gpu_cache_usage_perc"
        
        # Check token processing metrics exist
        assert "vllm:prompt_tokens_total" in rules
        assert rules["vllm:prompt_tokens_total"].new_name == "aibrix:prompt_tokens_total"
        
        assert "vllm:generation_tokens_total" in rules
        assert rules["vllm:generation_tokens_total"].new_name == "aibrix:generation_tokens_total"
        
        # Check latency metrics exist
        assert "vllm:time_to_first_token_seconds" in rules
        assert rules["vllm:time_to_first_token_seconds"].new_name == "aibrix:time_to_first_token_seconds"
        
        assert "vllm:time_per_output_token_seconds" in rules
        assert rules["vllm:time_per_output_token_seconds"].new_name == "aibrix:time_per_output_token_seconds"
        
        assert "vllm:e2e_request_latency_seconds" in rules
        assert rules["vllm:e2e_request_latency_seconds"].new_name == "aibrix:e2e_request_latency_seconds"
        
        # Check success metrics exist
        assert "vllm:request_success_total" in rules
        assert rules["vllm:request_success_total"].new_name == "aibrix:request_success_total"
    ...
    def test_get_metric_rules_sglang(self):
        ...
        assert rules["sglang:prompt_tokens_total"].new_name == "aibrix:prompt_tokens_total"
        
        assert "sglang:generation_tokens_total" in rules
        assert rules["sglang:generation_tokens_total"].new_name == "aibrix:generation_tokens_total"
        
        # Check latency metrics exist
        assert "sglang:time_to_first_token_seconds" in rules
        assert rules["sglang:time_to_first_token_seconds"].new_name == "aibrix:time_to_first_token_seconds"
        
        assert "sglang:time_per_output_token_seconds" in rules
        assert rules["sglang:time_per_output_token_seconds"].new_name == "aibrix:time_per_output_token_seconds"
        
        assert "sglang:e2e_request_latency_seconds" in rules
        assert rules["sglang:e2e_request_latency_seconds"].new_name == "aibrix:e2e_request_latency_seconds"
    ...
    def test_core_metrics_standardization(self):
        ...
        assert vllm_queue_rule.new_name == sglang_queue_rule.new_name == "aibrix:queue_size"
        ...
        assert vllm_prompt_rule.new_name == sglang_prompt_rule.new_name == "aibrix:prompt_tokens_total"
        
        vllm_gen_rule = vllm_rules["vllm:generation_tokens_total"]
        sglang_gen_rule = sglang_rules["sglang:generation_tokens_total"]
        assert vllm_gen_rule.new_name == sglang_gen_rule.new_name == "aibrix:generation_tokens_total"
        ...
        assert vllm_ttft_rule.new_name == sglang_ttft_rule.new_name == "aibrix:time_to_first_token_seconds"
        ...
        assert vllm_tpot_rule.new_name == sglang_tpot_rule.new_name == "aibrix:time_per_output_token_seconds"
        
        vllm_e2e_rule = vllm_rules["vllm:e2e_request_latency_seconds"]
        sglang_e2e_rule = sglang_rules["sglang:e2e_request_latency_seconds"]
        assert vllm_e2e_rule.new_name == sglang_e2e_rule.new_name == "aibrix:e2e_request_latency_seconds"
...
class TestPassthroughStandardRule:
    ...
    @staticmethod
    def create_sample_metric(name: str, value: float = 1.0, metric_type: str = "counter"):
    ...
    def test_passthrough_unchanged(self):
        ...
        assert returned_metric.samples[0].value == original_metric.samples[0].value == 42.0
...
class TestHybridMappingStrategy:
    ...
    def test_vllm_hybrid_coverage(self):
        ...
        rename_rules = sum(1 for r in rules.values() if isinstance(r, RenameStandardRule))
        passthrough_rules = sum(1 for r in rules.values() if isinstance(r, PassthroughStandardRule))
        
        # Should have both types of rules
        assert rename_rules > 0, "Should have some renamed metrics for standardization"
        assert passthrough_rules > 0, "Should have some pass-through metrics for debugging"
