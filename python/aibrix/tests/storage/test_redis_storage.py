...
def test_multipart_not_supported():
    """Test that multipart operations raise NotImplementedError."""
    storage = RedisStorage()

    with pytest.raises(NotImplementedError):
        storage.create_multipart_upload("test", None, None)

    with pytest.raises(NotImplementedError):
        storage.upload_part("test", "upload_id", 1, b"data")

    with pytest.raises(NotImplementedError):
        storage.complete_multipart_upload("test", "upload_id", [])

    with pytest.raises(NotImplementedError):
        storage.abort_multipart_upload("test", "upload_id")


def test_head_object_not_supported():
    """Test that head_object raises NotImplementedError."""
    storage = RedisStorage()

    with pytest.raises(NotImplementedError):
        storage.head_object("test")


# Note: Integration tests requiring actual Redis connection would go here
# but are commented out since they require Redis to be running
...
# Example of how timestamp ordering would work:
...
#         remaining, remaining_token = await storage.list_objects(limit=5, continuation_token=limited_token)
...
#         page2, token2 = await storage.list_objects("batch", "/", limit=4, continuation_token=token1)
#
#         assert len(page1) == 4
#         assert len(page2) == 4
#
...
#         objects = await storage.list_objects("batch", "/")
#
