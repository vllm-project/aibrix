...
kv_cache_ol_environment_variables: Dict[str, Callable[[], Any]] = {
    ...
    # ================== PrisKV Env Vars ==================
    "AIBRIX_KV_CACHE_OL_PRISKV_REMOTE_ADDR": lambda: (
        os.getenv(
            "AIBRIX_KV_CACHE_OL_PRISKV_REMOTE_ADDR",
            os.getenv("AIBRIX_KV_CACHE_OL_PRIS_REMOTE_ADDR", "127.0.0.1"),
        ).strip()
    ),
    "AIBRIX_KV_CACHE_OL_PRISKV_REMOTE_PORT": lambda: int(
        os.getenv(
            "AIBRIX_KV_CACHE_OL_PRISKV_REMOTE_PORT",
            os.getenv("AIBRIX_KV_CACHE_OL_PRIS_REMOTE_PORT", "6379"),
        )
    ),
    "AIBRIX_KV_CACHE_OL_PRISKV_USE_MPUT_MGET": lambda: (
        os.getenv(
            "AIBRIX_KV_CACHE_OL_PRISKV_USE_MPUT_MGET",
            os.getenv("AIBRIX_KV_CACHE_OL_PRIS_USE_MPUT_MGET", "0"),
        )
        .strip()
        .lower()
        in ("1", "true")
    ),
    "AIBRIX_KV_CACHE_OL_PRISKV_PASSWORD": lambda: (
        os.getenv(
